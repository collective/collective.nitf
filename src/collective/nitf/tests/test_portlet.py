...
class PortletTest(unittest.TestCase):
    ...
    def test_portlet_type_registered(self):
        ...
        self.assertEquals(latest_sectionable.addview,
                          'collective.nitf.LatestSectionableNITFPortlet')

    def test_interfaces(self):
        latest_sectionable = latest_sectionable_nitf.Assignment()
        
        self.failUnless(IPortletAssignment.providedBy(latest_sectionable))
        self.failUnless(IPortletDataProvider.providedBy(latest_sectionable.data))
    ...
    def test_invoke_edit_view(self):
        ...
        self.failUnless(isinstance(editview, latest_sectionable_nitf.EditForm))
    ...
    def test_obtain_renderer(self):
        ...
        self.failUnless(isinstance(renderer1, latest_sectionable_nitf.Renderer))
...
class RenderTest(unittest.TestCase):
    ...
    def test_default_search_for_nitf(self):
        ...
        self.failUnlessEqual(len(results), 10)
        query = self.default_query
                 
        catalog_results = self.pc(**query)[:10]

        self.failUnlessEqual([i.id for i in results],
                             [i.id for i in catalog_results])

        results = view.getResults(limit=45)

        self.failUnlessEqual(len(results), 45)
    ...
    def test_filter_sections(self):
        ...
        for section in [("section 1", "Section 1"),
            ...
            self.failUnlessEqual(len(results), 15)
            query = self.default_query
            query['sort_limit'] = 45
    ...
    def test_modifying_query_through_collection(self):
        ...
        self.failUnlessEqual(len(results), 9)
        query = self.default_query
